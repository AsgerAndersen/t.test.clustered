...
#' to the model explained in XXXX with parameters defined by the arguments of 
...
#' cluster adjusted t statistic and \code{\link{power.t.test.clustered}}
#' for power calculation of the cluster adjusted t test.
#' 
#' @export 
simulate.power.t.test.clustered <- function(num_experiments=1000, clusters_group_1, 
  ...
  for (j in 1:num_experiments) {
    ...
    accepted <- accepted + t.test.clustered(test_data,alt,sig_level)
  }
...
data_generate_experiment <- function(num_obs_clus_group_1, num_obs_clus_group_2=num_obs_clus_group_1, 
                                     delta, sd = 1, rho) {
  responsee_group_1 <- data_generate_group(num_obs_clus_group_1,group_mean=0,sd_group=sd,rho)
